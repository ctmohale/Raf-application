...
function groupedTextLines(value, font, fontSize, maxWidth, maxLines) {
  ...
  for (const explicitLine of explicitLines) {
    ...
    if (wrapped.length) lines.push(...wrapped);else lines.push('');
  }
  return lines.slice(0, maxLines);
}
function groupedInputBoxes(field) {
  const groupConfig = Array.isArray(field.options) ? field.options.find(option => option && option.kind === 'grouped-input-boxes') : null;
  if (!Array.isArray(groupConfig?.boxes) || groupConfig.boxes.length === 0) return [];
  return groupConfig.boxes.filter(box => Number.isFinite(Number(box.x)) && Number.isFinite(Number(box.y))).sort((a, b) => Number(a.y) - Number(b.y) || Number(a.x) - Number(b.x));
}
...
function checkboxMarkStyle(field) {
  const option = Array.isArray(field?.options) ? field.options.find(item => item && item.kind === 'checkbox-mark-style') : null;
  return option?.value === 'check' ? 'check' : 'x';
}
function drawCheckMark(page, {
  x,
  y,
  size,
  color
...
export function validateDataAgainstFields(fields, data, {
  rejectUnknown = false
} = {}) {
  const errors = [];
...
export async function generateFilledPdf({
  ...
  for (const field of fields) {
    ...
    if (signatureImage) {
      const embeddedImage = signatureImage.type === 'jpg' ? await pdfDoc.embedJpg(signatureImage.bytes) : await pdfDoc.embedPng(signatureImage.bytes);
  ...
  return serializeDocument(await db.prepare(`
