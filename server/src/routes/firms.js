...
import { coreClientDocumentRequests, dropFirmDatabase, getFirmDatabaseName, initializeFirmDatabase, openFirmDatabase, slugifyFirmName } from '../services/firmDatabases.js';
...
async function getFirmTeamMembers(firmId, firmDb = null) {
...
function serializeFirm(row) {
  return {
    ...
    database_name: getFirmDatabaseName(row.database_filename),
...
function safeJson(value) {
  const compacted = Array.isArray(value) ? value.map(compactObject).filter(item => Object.keys(item).length) : compactObject(value);
...
function determineRequiredForms(accidentDate, claimAmounts = {}) {
  ...
  return {
    ...
    notes: [hasGeneralDamages ? 'RAF 4 is required because general damages are being claimed.' : 'RAF 4 may be required if general damages or a serious injury assessment will be pursued.', 'RAF 2 applies only to supplier claims.']
  };
}
...
async function nextAvailableSlug(name) {
  const baseSlug = slugifyFirmName(name);
  let slug = baseSlug;
  let suffix = 2;
  while (await db.prepare('SELECT id FROM firms WHERE slug = ?').get(slug)) {
    slug = `${baseSlug}-${suffix}`;
    suffix += 1;
  }
  return slug;
}
async function getFirmOr404(identifier, res) {
  const firm = await db.prepare('SELECT * FROM firms WHERE id = ? OR slug = ?').get(identifier, identifier);
  if (!firm) {
    res.status(404).json({
...
async function getWorkspacePayload(firm, req) {
  ...
  try {
    ...
    `).all()).map(client => {
      const reminderDue = Boolean(client.auto_reminders_enabled && Number(client.pending_documents || 0) > 0 && client.next_reminder_at && new Date(client.next_reminder_at) <= new Date());
      return serializeFirmClient({
        ...client,
    ...
    const cases = await firmDb.prepare(`
      ...
      LIMIT 25
    `).all();
    const teamMembers = await getFirmTeamMembers(firm.id, firmDb);
    const teamById = new Map(teamMembers.map(member => [Number(member.id), member]));
    const matters = (await getMatterRows(firmDb)).map(matter => serializeMatter(matter, teamById));
    const enrichedCases = cases.map(caseRecord => ({
      ...caseRecord,
      linked_matter: caseRecord.matter_id ? matters.find(matter => Number(matter.id) === Number(caseRecord.matter_id)) || null : null,
    ...
    const documentRequests = await firmDb.prepare(`
      SELECT
    ...
    `).all();
    const medicalAssessmentRequests = (await firmDb.prepare(`
      SELECT
    ...
    const doctors = (await firmDb.prepare(`
      SELECT *
      FROM firm_doctors
      ORDER BY
    ...
    const visibleMatters = canViewAllMatters ? matters : matters.filter(matter => Number(matter.responsible_lawyer_user_id) === Number(req.user?.id) || Number(matter.assigned_assistant_user_id) === Number(req.user?.id));
    const visibleMatterIds = new Set(visibleMatters.map(matter => Number(matter.id)));
    const visibleCases = canViewAllMatters ? enrichedCases : enrichedCases.filter(caseRecord => visibleMatterIds.has(Number(caseRecord.matter_id)) || Number(caseRecord.responsible_lawyer_user_id) === Number(req.user?.id) || Number(caseRecord.assigned_assistant_user_id) === Number(req.user?.id));
    ...
    const visibleClients = canViewAllMatters ? clients : clients.filter(client => visibleClientIds.has(Number(client.id)));
    const visibleDocumentRequests = canViewAllMatters ? documentRequests : documentRequests.filter(request => visibleCaseIds.has(Number(request.case_id)));
    const visibleMedicalAssessmentRequests = canViewAllMatters ? medicalAssessmentRequests : medicalAssessmentRequests.filter(request => visibleCaseIds.has(Number(request.case_id)));
    const claimForms = await getClaimFormAttachments(firmDb);
    const visibleClaimForms = canViewAllMatters ? claimForms : claimForms.filter(form => visibleCaseIds.has(Number(form.case_id)));
    const stats = {
      clients: (await firmDb.prepare('SELECT COUNT(*) AS count FROM firm_clients').get()).count,
      openCases: (await firmDb.prepare("SELECT COUNT(*) AS count FROM raf_cases WHERE status != 'closed'").get()).count,
      openMatters: (await firmDb.prepare("SELECT COUNT(*) AS count FROM matters WHERE status != 'closed'").get()).count,
      requestedDocuments: (await firmDb.prepare(`
        SELECT COUNT(*) AS count
        FROM client_document_requests
      ...
      `).get()).count,
      uploadedDocuments: (await firmDb.prepare(`
        SELECT COUNT(*) AS count
        FROM client_document_requests
        WHERE ${clientUploadDocumentCondition}
          AND (
    ...
    if (!canViewAllMatters) {
      ...
      stats.requestedDocuments = visibleDocumentRequests.filter(request => request.status !== 'uploaded' && Number(request.upload_count || 0) === 0).length;
      stats.uploadedDocuments = visibleDocumentRequests.filter(request => request.status === 'uploaded' || Number(request.upload_count || 0) > 0).length;
...
async function createDefaultDocumentRequests(firmDb, clientId, caseId, context = {}) {
  ...
  for (const [type, label, instructions] of coreClientDocumentRequests) {
    await insert.run(clientId, caseId, type, label, instructions);
  }
}
...
async function getReminderDueCount(firm) {
  const firmDb = await openFirmDatabase(firm);
  try {
    const rows = await firmDb.prepare(`
...
async function getFirmMessageThread(firmId) {
  return (await db.prepare(`
    SELECT *
    FROM firm_messages
    WHERE firm_id = ?
...
function buildClientUploadEmail({
  firm,
  client,
  subject = null,
  body = null
...
function readableFieldLabel(field, fields) {
  ...
  const candidates = fields.filter(candidate => candidate.id !== field.id && candidate.page_number === field.page_number && !isGenericFieldLabel(candidate.label) && Number(candidate.y || 0) <= y + 8).sort((a, b) => Number(b.y || 0) - Number(a.y || 0));
...
async function getAvailableClaimTemplates() {
  return (await db.prepare(`
...
async function findMedicalTemplateForReport(reportType) {
...
async function serializeClaimFormAttachment(row) {
...
function serializeMatter(row, teamById = new Map()) {
  if (!row) return null;
  const linkedClaim = row.linked_raf_case_id ? buildClaimWorkflowSummary(row) : null;
  return {
    id: row.id,
...
async function getMatterRows(firmDb) {
...
async function getClaimFormAttachments(firmDb, caseId = null) {
  const rows = caseId ? await firmDb.prepare('SELECT * FROM claim_form_templates WHERE case_id = ? ORDER BY created_at DESC, id DESC').all(caseId) : await firmDb.prepare('SELECT * FROM claim_form_templates ORDER BY created_at DESC, id DESC').all();
  return rows.map(serializeClaimFormAttachment);
}
async function getClaimWithClient(firmDb, caseId) {
  return await firmDb.prepare(`
    SELECT
      raf_cases.*,
      matters.matter_reference,
...
function buildClaimTemplateData({
  firm,
  claim,
  fields
}) {
  ...
  for (const aiDocument of aiDocuments) {
    ...
    const claimant = Array.isArray(extraction.parties) ? extraction.parties.find(party => /claimant|client|patient|injured/i.test(party?.role || '')) || extraction.parties[0] || {} : {};
...
async function generateClaimFormDocument({
  firm,
  claim,
  template,
  userId,
  ...
  const data = dataOverride || buildClaimTemplateData({
    firm,
    claim,
    fields
  });
...
async function getUserFirmAccess(userId, firmId) {
  return await db.prepare(`
    SELECT *
    FROM user_firm_access
...
function requireFirmAccess(options = {}) {
  return async (req, res, next) => {
    const firm = await getFirmOr404(req.params.id, res);
    if (!firm) return;
    req.firm = firm;
    if (req.user?.role === 'admin') return next();
    const access = await getUserFirmAccess(req.user.id, firm.id);
    ...
    if (options.write && (req.user?.role === 'viewer' || access.access_level === 'viewer')) {
      return res.status(403).json({
        error: 'Viewer access cannot change firm records'
      });
    }
...
function requireAssignedMatter(options = {}) {
  return async (req, res, next) => {
    ...
    const firmDb = await openFirmDatabase(req.firm);
    try {
      let permitted = null;
      if (options.caseParam) {
        permitted = await firmDb.prepare(`
          SELECT raf_cases.id
          FROM raf_cases
          LEFT JOIN matters ON matters.id = raf_cases.matter_id
          WHERE raf_cases.id = ?
      ...
      if (!permitted) return res.status(403).json({
        error: 'This matter is not assigned to you'
      });
      return next();
    } finally {
      await firmDb.close();
...
firmsRouter.get('/', requireAdminOnly, async (_req, res) => {
...
firmsRouter.get('/messages/overview', async (req, res) => {
  const firms = req.user?.role === 'admin' ? await db.prepare('SELECT * FROM firms ORDER BY name COLLATE NOCASE').all() : await db.prepare(`
        SELECT firms.*
        FROM firms
      ...
      `).all(req.user.id);
  const rows = await Promise.all(firms.map(async firm => {
    const latestMessage = await db.prepare(`
      ...
      LIMIT 1
    `).get(firm.id);
    const unreadAdmin = (await db.prepare(`
      SELECT COUNT(*) AS count
      FROM firm_messages
      WHERE firm_id = ? AND sender_type = 'firm' AND read_by_admin = 0
    `).get(firm.id)).count;
    const unreadFirm = (await db.prepare(`
      SELECT COUNT(*) AS count
      FROM firm_messages
    ...
    return {
      ...
      reminders_due: await getReminderDueCount(firm)
    };
  }));
  res.json({
    rows,
...
firmsRouter.get('/:id/messages', requireFirmAccess(), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  res.json({
    firm: serializeFirm(firm),
    messages: await getFirmMessageThread(firm.id)
  });
});
firmsRouter.post('/:id/messages', requireFirmAccess({
  write: true
}), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  ...
  const senderType = req.user?.role === 'admin' ? req.body?.sender_type === 'firm' ? 'firm' : 'admin' : 'firm';
  ...
  await db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(firm.id, senderType, senderName, body, senderType === 'admin' ? 1 : 0, senderType === 'firm' ? 1 : 0);
  res.status(201).json({
    firm: serializeFirm(firm),
    messages: await getFirmMessageThread(firm.id)
  });
});
firmsRouter.patch('/:id/messages/read', requireFirmAccess(), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  const readerType = req.user?.role === 'admin' ? req.body?.reader_type === 'firm' ? 'firm' : 'admin' : 'firm';
  if (readerType === 'firm') {
    await db.prepare(`
...
firmsRouter.post('/', requireAdminOnly, async (req, res) => {
  const {
    name,
  ...
  const createFirm = db.transaction(async () => {
    const slug = await nextAvailableSlug(name);
    ...
    const result = await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(String(name).trim(), slug, contactName ? String(contactName).trim() : null, contactEmail ? String(contactEmail).trim().toLowerCase() : null, contactPhone ? String(contactPhone).trim() : null, address ? String(address).trim() : null, databaseFilename, req.user.id);
    const firm = await db.prepare('SELECT * FROM firms WHERE id = ?').get(result.lastInsertRowid);
    await initializeFirmDatabase(firm);
    return firm;
  });
...
firmsRouter.get('/:id/workspace', requireFirmAccess(), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  res.json(await getWorkspacePayload(firm, req));
});
firmsRouter.get('/:id/team', requireFirmAccess(), async (req, res) => {
  const firm = req.firm;
  const firmDb = await openFirmDatabase(firm);
  try {
    return res.json({
      ...
      members: await getFirmTeamMembers(firm.id, firmDb)
    });
  } finally {
    await firmDb.close();
  }
});
firmsRouter.post('/:id/team', requireFirmAccess({
  write: true
}), async (req, res, next) => {
  const firm = req.firm;
  if (!canManageFirmTeam(req)) return res.status(403).json({
    error: 'Firm Admin access is required to add team members'
  });
  ...
  if (!firmTeamRoles.has(firmRole)) return res.status(400).json({
    error: 'Choose Firm Admin, Lawyer, or Assistant'
  });
  if (!teamAccountStatuses.has(status)) return res.status(400).json({
    error: 'Choose a valid account status'
  });
  if (!sendInvitation && suppliedPassword.length < 8) return res.status(400).json({
    error: 'Temporary password must be at least 8 characters'
  });
  const existingUser = await db.prepare('SELECT * FROM users WHERE email = ?').get(email);
  if (existingUser?.role === 'admin') return res.status(409).json({
    error: 'This email belongs to a workspace administrator'
  });
  if (existingUser) {
    const otherAccess = await db.prepare('SELECT firm_id FROM user_firm_access WHERE user_id = ? AND firm_id != ?').get(existingUser.id, firm.id);
    if (otherAccess) return res.status(409).json({
      error: 'This user already belongs to another law firm'
    });
    const sameAccess = await db.prepare('SELECT id FROM user_firm_access WHERE user_id = ? AND firm_id = ?').get(existingUser.id, firm.id);
    if (sameAccess) return res.status(409).json({
      error: 'This user is already a member of this firm'
    });
  }
  ...
  const createMember = db.transaction(async () => {
  ...
  try {
    return res.status(201).json({
      member: (await getFirmTeamMembers(firm.id, firmDb)).find(member => Number(member.id) === Number(userId)),
      members: await getFirmTeamMembers(firm.id, firmDb),
      invitation,
...
firmsRouter.patch('/:id/team/:userId', requireFirmAccess({
  write: true
}), async (req, res) => {
  const firm = req.firm;
  if (!canManageFirmTeam(req)) return res.status(403).json({
    error: 'Firm Admin access is required to edit team members'
  });
  const member = await getFirmMember(firm.id, req.params.userId);
  ...
  const duplicate = await db.prepare('SELECT id FROM users WHERE email = ? AND id != ?').get(email, member.id);
  ...
  if (member.firm_role === 'firm_admin' && (firmRole !== 'firm_admin' || status !== 'active')) {
    ...
    if (Number(otherAdmins) < 1) return res.status(400).json({
      error: 'At least one active Firm Admin is required'
    });
  }
  const updateMember = db.transaction(async () => {
    if (newPassword) {
      await db.prepare(`UPDATE users SET name = ?, email = ?, status = ?, password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(name, email, status, bcrypt.hashSync(newPassword, 12), member.id);
    } else {
      await db.prepare(`UPDATE users SET name = ?, email = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).run(name, email, status, member.id);
    }
    await db.prepare(`
  ...
  try {
    return res.json({
      member: (await getFirmTeamMembers(firm.id, firmDb)).find(row => Number(row.id) === Number(member.id)),
      members: await getFirmTeamMembers(firm.id, firmDb)
    });
  } finally {
    await firmDb.close();
...
async function getVisibleMattersPayload(firm, req) {
  const firmDb = await openFirmDatabase(firm);
  try {
    const teamMembers = await getFirmTeamMembers(firm.id, firmDb);
    const teamById = new Map(teamMembers.map(member => [Number(member.id), member]));
    const canViewAllMatters = req.user?.role === 'admin' || req.firmAccess?.firm_role === 'firm_admin';
    const matters = (await getMatterRows(firmDb)).map(matter => serializeMatter(matter, teamById)).filter(matter => canViewAllMatters || Number(matter.responsible_lawyer_user_id) === Number(req.user?.id) || Number(matter.assigned_assistant_user_id) === Number(req.user?.id));
...
firmsRouter.get('/:id/matters', requireFirmAccess(), async (req, res) => {
  return res.json(await getVisibleMattersPayload(req.firm, req));
});
firmsRouter.get('/:id/my-matters', requireFirmAccess(), async (req, res) => {
  return res.json(await getVisibleMattersPayload(req.firm, req));
});
firmsRouter.get('/:id/matters/:matterId', requireFirmAccess(), requireAssignedMatter({
  ...
  try {
    const teamMembers = await getFirmTeamMembers(firm.id, firmDb);
    const teamById = new Map(teamMembers.map(member => [Number(member.id), member]));
    const row = (await getMatterRows(firmDb)).find(matter => Number(matter.id) === Number(req.params.matterId));
    if (!row) return res.status(404).json({
      error: 'Matter not found'
    });
    const matter = serializeMatter(row, teamById);
    const claim = matter.linked_raf_case_id ? await getClaimWithClient(firmDb, matter.linked_raf_case_id) : null;
    return res.json({
      firm: serializeFirm(firm),
      matter,
      linked_claim: claim ? {
      ...
      activity_history: await firmDb.prepare(`
...
firmsRouter.patch('/:id/matters/:matterId/assignment', requireFirmAccess({
  write: true
}), async (req, res) => {
  const firm = req.firm;
  if (!canManageFirmTeam(req)) return res.status(403).json({
    error: 'Firm Admin access is required to assign matters'
  });
  ...
  if (!lawyerId) return res.status(400).json({
    error: 'Choose a responsible lawyer'
  });
  const lawyer = await getFirmMember(firm.id, lawyerId);
  const assistant = assistantId ? await getFirmMember(firm.id, assistantId) : null;
  if (!lawyer || lawyer.status !== 'active' || !['lawyer', 'firm_admin'].includes(lawyer.firm_role)) {
    return res.status(400).json({
      error: 'Choose an active lawyer from this firm'
    });
  }
  if (assistantId && (!assistant || assistant.status !== 'active' || assistant.firm_role !== 'assistant')) {
    return res.status(400).json({
      error: 'Choose an active assistant from this firm'
    });
  }
  const firmDb = await openFirmDatabase(firm);
  try {
    const matter = await firmDb.prepare('SELECT * FROM matters WHERE id = ?').get(req.params.matterId);
    if (!matter) return res.status(404).json({
      error: 'Matter not found'
    });
    const isReassignment = Boolean(matter.responsible_lawyer_user_id || matter.assigned_assistant_user_id);
    const changed = Number(matter.responsible_lawyer_user_id || 0) !== Number(lawyerId || 0) || Number(matter.assigned_assistant_user_id || 0) !== Number(assistantId || 0);
    if (!changed) return res.status(400).json({
      error: 'Choose a different lawyer or assistant to reassign this matter'
    });
    if (isReassignment && !reason) return res.status(400).json({
      error: 'Add a reason for reassignment'
    });
    const assignMatter = firmDb.transaction(async () => {
      ...
      await firmDb.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(matter.id, matter.linked_raf_case_id || null, matter.responsible_lawyer_user_id || null, matter.assigned_assistant_user_id || null, lawyerId, assistantId, req.user.id, assignmentNote || null, reason || null);
    });
    await assignMatter();
    ...
    await addAssignmentNotification({
      userId: lawyerId,
      firmId: firm.id,
      caseId: matter.linked_raf_case_id || null,
      message: `${matter.matter_reference} was ${action} to you as responsible lawyer.`
    });
    if (assistantId) await addAssignmentNotification({
      userId: assistantId,
      firmId: firm.id,
      caseId: matter.linked_raf_case_id || null,
      message: `${matter.matter_reference} was ${action} to you as assigned assistant.`
    });
    return res.json(await getVisibleMattersPayload(firm, req));
  } finally {
    await firmDb.close();
  }
});
firmsRouter.patch('/:id/claims/:caseId/assignment', requireFirmAccess({
  write: true
}), async (req, res) => {
  const firm = req.firm;
  if (!canManageFirmTeam(req)) return res.status(403).json({
  ...
  const firmDb = await openFirmDatabase(firm);
  try {
    const claim = await firmDb.prepare(`
    ...
    const changed = Number(currentLawyerId || 0) !== Number(lawyerId || 0) || Number(currentAssistantId || 0) !== Number(assistantId || 0);
    if (!changed) return res.status(400).json({
      error: 'Choose a different lawyer or assistant to reassign this matter'
    });
    if (isReassignment && !reason) return res.status(400).json({
      ...
      await firmDb.prepare(`
        INSERT INTO matter_assignment_history
          (matter_id, case_id, previous_lawyer_user_id, previous_assistant_user_id,
           ...
           assignment_note, reason)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(claim.linked_matter_id || claim.matter_id || null, claim.id, currentLawyerId || null, currentAssistantId || null, lawyerId, assistantId, req.user.id, assignmentNote || null, reason || null);
    });
    await assignMatter();
    const action = isReassignment ? 'reassigned' : 'assigned';
    await addAssignmentNotification({
      ...
      firmId: firm.id,
      caseId: claim.id,
      message: `${claim.case_reference} was ${action} to you as responsible lawyer.`
    });
    if (assistantId) await addAssignmentNotification({
      userId: assistantId,
      firmId: firm.id,
      caseId: claim.id,
      message: `${claim.case_reference} was ${action} to you as assigned assistant.`
    });
    return res.json({
      workspace: await getWorkspacePayload(firm, req),
      history: await firmDb.prepare('SELECT * FROM matter_assignment_history WHERE case_id = ? ORDER BY created_at DESC, id DESC').all(claim.id)
    });
  } finally {
    await firmDb.close();
  }
});
firmsRouter.post('/:id/doctors', requireFirmAccess({
  write: true
}), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  ...
  const firmDb = await openFirmDatabase(firm);
  try {
    const result = await firmDb.prepare(`
      INSERT INTO firm_doctors
        (full_name, practice_number, email, phone, specialty, relationship_notes, status)
      VALUES (?, ?, ?, ?, ?, ?, 'active')
    `).run(fullName, practiceNumber || null, email || null, phone || null, specialty || null, relationshipNotes || null);
    const doctor = serializeFirmDoctor(await firmDb.prepare('SELECT * FROM firm_doctors WHERE id = ?').get(result.lastInsertRowid));
    return res.status(201).json({
      doctor,
      workspace: await getWorkspacePayload(firm, req)
    });
...
firmsRouter.patch('/:id/doctors/:doctorId', requireFirmAccess({
  write: true
}), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  ...
  if (!['active', 'inactive'].includes(status)) return res.status(400).json({
    error: 'Choose active or inactive status'
  });
  const firmDb = await openFirmDatabase(firm);
  try {
    const existing = await firmDb.prepare('SELECT * FROM firm_doctors WHERE id = ?').get(req.params.doctorId);
    ...
    await firmDb.prepare(`
      ...
      WHERE id = ?
    `).run(fullName, practiceNumber || null, email || null, phone || null, specialty || null, relationshipNotes || null, status, existing.id);
    const doctor = serializeFirmDoctor(await firmDb.prepare('SELECT * FROM firm_doctors WHERE id = ?').get(existing.id));
    return res.json({
      doctor,
      workspace: await getWorkspacePayload(firm, req)
    });
...
firmsRouter.delete('/:id/doctors/:doctorId', requireFirmAccess({
  write: true
}), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  ...
  try {
    ...
    await firmDb.prepare('DELETE FROM firm_doctors WHERE id = ?').run(existing.id);
    return res.json({
      deleted: true,
...
firmsRouter.get('/:id/templates', requireFirmAccess(), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  const firmDb = await openFirmDatabase(firm);
  try {
    const templates = (await firmDb.prepare(`
      SELECT document_templates.*, COUNT(template_fields.id) AS field_count
      FROM document_templates
      LEFT JOIN template_fields ON template_fields.template_id = document_templates.id
      GROUP BY document_templates.id
...
firmsRouter.get('/:id/documents', requireFirmAccess(), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  const firmDb = await openFirmDatabase(firm);
  try {
    const documents = (await firmDb.prepare(`
      SELECT generated_documents.*, document_templates.name AS template_name
      FROM generated_documents
      JOIN document_templates ON document_templates.id = generated_documents.template_id
...
firmsRouter.get('/:id/claims/:caseId/forms', requireFirmAccess(), requireAssignedMatter({
  caseParam: 'caseId'
}), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  const firmDb = await openFirmDatabase(firm);
  try {
    const claim = await getClaimWithClient(firmDb, req.params.caseId);
    if (!claim) return res.status(404).json({
    ...
    res.json({
      firm: serializeFirm(firm),
      claim,
      available_templates: await getAvailableClaimTemplates(),
      attached_forms: await getClaimFormAttachments(firmDb, claim.id)
    });
  } finally {
    await firmDb.close();
  }
});
firmsRouter.post('/:id/claims/:caseId/medical-assessments', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  caseParam: 'caseId'
}), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  ...
  const clientOrigin = getRequestClientOrigin(req);
  if (!medicalReportTypes.has(reportType)) return res.status(400).json({
    error: 'Choose a valid medical report type'
  });
  if (!doctorName) return res.status(400).json({
    error: 'Doctor name is required'
  });
  if (deliveryMethod !== 'link' && !doctorEmail && !doctorPhone) return res.status(400).json({
    error: 'Doctor email or phone is required'
  });
  if (!['link', 'email', 'sms'].includes(deliveryMethod)) return res.status(400).json({
    error: 'Choose link, email, or SMS delivery'
  });
  const firmDb = await openFirmDatabase(firm);
  try {
    const claim = await getClaimWithClient(firmDb, req.params.caseId);
    ...
    const result = await firmDb.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'requested', ?, ?, ?)
    `).run(claim.id, claim.client_id, reportType, doctorName, practiceNumber || null, doctorEmail || null, doctorPhone || null, deadline || null, deliveryMethod, token, inheritClientInformation ? 1 : 0, lockPrefilledFields ? 1 : 0, hidePrefilledFields ? 1 : 0);
    let request = serializeMedicalAssessmentRequest(await firmDb.prepare('SELECT * FROM medical_assessment_requests WHERE id = ?').get(result.lastInsertRowid), clientOrigin);
    let delivery = {
      sent: false,
    ...
    if (deliveryMethod === 'email' && doctorEmail) {
      try {
        ...
        await firmDb.prepare(`
          ...
          WHERE id = ?
        `).run(request.id);
        request = serializeMedicalAssessmentRequest(await firmDb.prepare('SELECT * FROM medical_assessment_requests WHERE id = ?').get(request.id), clientOrigin);
        delivery = {
          sent: true,
          method: deliveryMethod
        };
...
firmsRouter.patch('/:id/claims/:caseId/medical-assessments/:requestId', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  caseParam: 'caseId'
}), async (req, res) => {
  ...
  try {
    ...
    const existing = await firmDb.prepare(`
    ...
    if (!existing) return res.status(404).json({
      error: 'Medical assessment request not found'
    });
    await firmDb.prepare(`
      UPDATE medical_assessment_requests
      SET
        ...
        updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `).run(reportType, doctorName, practiceNumber || null, doctorEmail || null, doctorPhone || null, deadline || null, deliveryMethod, inheritClientInformation ? 1 : 0, lockPrefilledFields ? 1 : 0, hidePrefilledFields ? 1 : 0, existing.id);
    if (refreshInheritedInformation && inheritClientInformation) {
      const template = await findMedicalTemplateForReport(reportType);
      if (template) {
        const fields = (await getTemplateFields(template.id)).map(serializeField);
        const latestInheritedValues = buildClaimTemplateData({
          firm,
          claim,
          fields
        });
    ...
    const request = serializeMedicalAssessmentRequest(await firmDb.prepare('SELECT * FROM medical_assessment_requests WHERE id = ?').get(existing.id), clientOrigin);
    return res.json({
      request,
      workspace: await getWorkspacePayload(firm, req)
    });
...
firmsRouter.post('/:id/claims/:caseId/forms', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  caseParam: 'caseId'
}), async (req, res, next) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  const templateIds = Array.isArray(req.body?.template_ids) ? req.body.template_ids.map(Number).filter(Boolean) : [Number(req.body?.template_id)].filter(Boolean);
  const uniqueTemplateIds = [...new Set(templateIds)];
  if (!uniqueTemplateIds.length) {
    return res.status(400).json({
      error: 'Choose at least one claim form template'
    });
  }
  const firmDb = await openFirmDatabase(firm);
  try {
    ...
    for (const templateId of uniqueTemplateIds) {
      const template = await db.prepare('SELECT * FROM document_templates WHERE id = ? AND status = ?').get(templateId, 'ready');
      if (!template) return res.status(404).json({
        error: `Template ${templateId} is not ready or was not found`
      });
      ...
      await firmDb.prepare(`
        ...
        ON DUPLICATE KEY UPDATE
          generated_document_id = VALUES(generated_document_id),
          status = 'generated',
          updated_at = CURRENT_TIMESTAMP
...
firmsRouter.post('/:id/claims/:caseId/forms/:attachmentId/ai-suggestions', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  caseParam: 'caseId'
}), async (req, res, next) => {
  ...
  try {
    ...
    const attachment = await firmDb.prepare('SELECT * FROM claim_form_templates WHERE id = ? AND case_id = ?').get(req.params.attachmentId, claim.id);
    if (!attachment) return res.status(404).json({
      error: 'Attached template form not found'
    });
    ...
    const pendingUploads = await firmDb.prepare(`
    ...
    for (const upload of pendingUploads) {
      const result = await processUploadedDocumentWithAi({
        firmDb,
        uploadId: upload.id
      });
    ...
    const refreshedClaim = await getClaimWithClient(firmDb, claim.id);
    const fields = (await getTemplateFields(template.id)).map(serializeField);
    const currentData = req.body?.data && typeof req.body.data === 'object' ? req.body.data : {};
    const generatedData = buildClaimTemplateData({
      firm,
    ...
    const mappedSuggestions = emptyFields.filter(field => hasManualValue(mergedData[field.name])).map(field => ({
    ...
    const suggestions = decisions.filter(decision => decision.status === 'fillable' && hasManualValue(decision.value));
    return res.json({
...
firmsRouter.post('/:id/claims/:caseId/forms/:attachmentId/fill-ai', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  caseParam: 'caseId'
}), async (req, res, next) => {
  ...
  const firmDb = await openFirmDatabase(firm);
  try {
    let claim = await getClaimWithClient(firmDb, req.params.caseId);
    if (!claim) return res.status(404).json({
      error: 'Claim not found'
    });
    const attachment = await firmDb.prepare('SELECT * FROM claim_form_templates WHERE id = ? AND case_id = ?').get(req.params.attachmentId, claim.id);
    ...
    const aiResults = [];
    for (const upload of pendingUploads) {
      aiResults.push(await processUploadedDocumentWithAi({
        firmDb,
        uploadId: upload.id
      }));
    }
    claim = await getClaimWithClient(firmDb, claim.id);
    const fields = (await getTemplateFields(template.id)).map(serializeField);
    const generatedData = buildClaimTemplateData({
      firm,
      claim,
    ...
    const finalAiProfile = parseJson(claim.ai_structured_json, {
      documents: []
    });
...
async function getAiAccessibleClaims(firmDb, req) {
  const canProcessAll = req.user?.role === 'admin' || req.firmAccess?.firm_role === 'firm_admin';
  return canProcessAll ? await firmDb.prepare('SELECT id FROM raf_cases ORDER BY id').all() : await firmDb.prepare(`
        SELECT raf_cases.id
        FROM raf_cases
        LEFT JOIN matters ON matters.id = raf_cases.matter_id
...
firmsRouter.get('/:id/claims/refresh-ai/plan', requireFirmAccess(), async (req, res, next) => {
  const firm = req.firm;
  const firmDb = await openFirmDatabase(firm);
  try {
    const claims = scopeAiClaims(await getAiAccessibleClaims(firmDb, req), req.query.case_id);
    const claimIds = claims.map(claim => Number(claim.id));
    if (!claimIds.length) return res.json({
      model: config.aiExtractionModel,
      documents: []
    });
...
firmsRouter.post('/:id/claims/refresh-ai/uploads/:uploadId', requireFirmAccess({
  write: true
}), async (req, res, next) => {
  const firm = req.firm;
  const firmDb = await openFirmDatabase(firm);
  try {
    const permittedCaseIds = new Set((await getAiAccessibleClaims(firmDb, req)).map(claim => Number(claim.id)));
    const upload = await firmDb.prepare(`
      SELECT
        client_uploads.id AS upload_id,
        client_uploads.original_filename,
        client_document_requests.label AS document_label,
    ...
    if (!upload) return res.status(404).json({
...
firmsRouter.post('/:id/claims/refresh-ai', requireFirmAccess({
  write: true
}), async (req, res, next) => {
  const firm = req.firm;
  const firmDb = await openFirmDatabase(firm);
  try {
    const claims = scopeAiClaims(await getAiAccessibleClaims(firmDb, req), req.body?.case_id);
    const summary = {
    ...
    for (const caseRecord of claims) {
      const uploads = req.body?.skip_uploads ? [] : await firmDb.prepare(`
        SELECT client_uploads.id
        FROM client_uploads
        JOIN client_document_requests ON client_document_requests.id = client_uploads.request_id
        WHERE client_document_requests.case_id = ?
      ...
      const claim = await getClaimWithClient(firmDb, caseRecord.id);
      if (!claim) continue;
      const attachments = await firmDb.prepare('SELECT * FROM claim_form_templates WHERE case_id = ? ORDER BY id').all(claim.id);
      for (const attachment of attachments) {
        try {
          ...
          const mappedResults = emptyBefore.filter(field => hasManualValue(mergedData[field.name])).map(field => ({
            field_name: field.name,
            label: field.label,
            status: 'fillable',
            value: mergedData[field.name],
          ...
          const inputsAdded = fields.reduce((count, field) => !hasManualValue(existingData[field.name]) && hasManualValue(reviewedData[field.name]) ? count + 1 : count, 0);
...
firmsRouter.post('/:id/claims/:caseId/forms/:attachmentId/fill-manual', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  caseParam: 'caseId'
}), async (req, res, next) => {
...
firmsRouter.patch('/:id/claims/:caseId/forms/:attachmentId/share', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  caseParam: 'caseId'
}), async (req, res) => {
  ...
  try {
    ...
    if (result.changes === 0) return res.status(404).json({
      error: 'Attached claim form not found'
    });
    return res.json({
      attached_forms: await getClaimFormAttachments(firmDb, claim.id),
      workspace: await getWorkspacePayload(firm, req)
...
firmsRouter.delete('/:id/claims/:caseId/forms/:attachmentId', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  caseParam: 'caseId'
}), async (req, res) => {
  ...
  try {
    ...
    const result = await firmDb.prepare('DELETE FROM claim_form_templates WHERE id = ? AND case_id = ?').run(req.params.attachmentId, claim.id);
    if (result.changes === 0) return res.status(404).json({
      error: 'Attached claim form not found'
    });
    return res.json({
...
firmsRouter.post('/:id/clients', requireFirmAccess({
  write: true
}), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  ...
  if (!resolvedFirstName || !resolvedSurname || !resolvedIdNumber || !cell || !email) {
    return res.status(400).json({
      error: 'First name, surname, ID number, cell, and email are required'
    });
  }
  if (!String(email).includes('@')) return res.status(400).json({
    error: 'Client email must be valid'
  });
  ...
  if (remindersEnabled && !reminderTime) {
    return res.status(400).json({
      error: 'Reminder time must use HH:MM format'
    });
  }
  ...
  if ((responsibleLawyerUserId || assignedAssistantUserId) && !canManageFirmTeam(req)) {
    return res.status(403).json({
      error: 'Firm Admin access is required to assign a new matter'
    });
  }
  const responsibleLawyer = responsibleLawyerUserId ? await getFirmMember(firm.id, responsibleLawyerUserId) : null;
  const assignedAssistant = assignedAssistantUserId ? await getFirmMember(firm.id, assignedAssistantUserId) : null;
  if (responsibleLawyerUserId && (!responsibleLawyer || responsibleLawyer.status !== 'active' || !['lawyer', 'firm_admin'].includes(responsibleLawyer.firm_role))) {
    return res.status(400).json({
      error: 'Choose an active responsible lawyer from this firm'
    });
  }
  ...
  if (assignedAssistantUserId && !responsibleLawyerUserId) return res.status(400).json({
    error: 'Choose a responsible lawyer before assigning an assistant'
  });
  const firmDb = await openFirmDatabase(firm);
  try {
    const createClient = firmDb.transaction(async () => {
      const clientResult = await firmDb.prepare(`
      ...
      `).run(resolvedFirstName, resolvedSurname, resolvedIdNumber, cleanString(passportNumber), cleanString(dateOfBirth) || null, cleanString(cell), String(email).trim().toLowerCase(), cleanString(residentialAddress), cleanString(occupation), cleanString(employerDetails), safeJson(banking), safeJson(representative), randomUUID(), remindersEnabled ? 1 : 0, reminderTime || '09:00', remindersEnabled ? getNextReminderAt(reminderTime) : null);
      ...
      const matterResult = await firmDb.prepare(`
        INSERT INTO matters (
      ...
      `).run(clientId, matterReference, matterTitle, responsibleLawyerUserId, assignedAssistantUserId, JSON.stringify([deadlineData.internal_deadline ? {
        label: 'Internal deadline',
        date: deadlineData.internal_deadline
      } : null, deadlineData.lodgement_deadline ? {
        label: 'RAF lodgement deadline',
        date: deadlineData.lodgement_deadline
      } : null].filter(Boolean)));
      const caseReference = `RAF-${firm.slug.toUpperCase()}-${String(clientId).padStart(4, '0')}`;
      const caseResult = await firmDb.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(clientId, matterResult.lastInsertRowid, caseReference, cleanString(accidentDate) || null, cleanString(accidentTime), cleanString(accidentLocation), cleanString(policeStation), cleanString(policeCaseNumber), cleanString(claimantRole), cleanString(collisionDescription), safeJson(vehicle), safeJson(driver), safeJson(owner), safeJson(witnesses), safeJson(compactClaimAmounts), formSelection.statutory_form_set, JSON.stringify(formSelection.required_forms), deadlineData.lodgement_deadline, deadlineData.internal_deadline, deadlineData.deadline_status, 'not_reviewed', JSON.stringify({
      ...
      }), responsibleLawyerUserId, assignedAssistantUserId);
      await firmDb.prepare(`
        UPDATE matters
      ...
      if (responsibleLawyerUserId) {
        await firmDb.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?)
        `).run(matterResult.lastInsertRowid, caseResult.lastInsertRowid, responsibleLawyerUserId, assignedAssistantUserId, req.user.id, cleanString(assignmentNoteInput) || null);
      }
      await createDefaultDocumentRequests(firmDb, clientId, caseResult.lastInsertRowid, {
        claimAmounts: compactClaimAmounts,
        requiredForms: formSelection.required_forms
      });
      return {
        client: await firmDb.prepare('SELECT * FROM firm_clients WHERE id = ?').get(clientId),
...
firmsRouter.post('/:id/clients/:clientId/invite', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  ...
  const firmDb = await openFirmDatabase(firm);
  try {
    const client = await firmDb.prepare('SELECT * FROM firm_clients WHERE id = ?').get(req.params.clientId);
    if (!client) return res.status(404).json({
      error: 'Client not found'
    });
    const email = buildClientUploadEmail({
      firm,
      client
    });
    ...
    const nextReminderAt = client.auto_reminders_enabled && (await hasPendingDocuments(firmDb, client.id)) ? getNextReminderAt(normalizeReminderTime(client.reminder_time) || '09:00') : null;
    const result = await firmDb.prepare(`
      UPDATE firm_clients
      SET
    ...
    const updatedClient = await firmDb.prepare('SELECT * FROM firm_clients WHERE id = ?').get(req.params.clientId);
    res.json({
      client: serializeFirmClient(updatedClient, config.clientOrigin),
      sent: true,
      provider: 'powermail',
...
firmsRouter.patch('/:id/clients/:clientId/portal-settings', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  clientParam: 'clientId'
}), async (req, res) => {
  ...
  try {
    ...
    const templatesVisible = req.body?.portal_templates_visible === undefined ? client.portal_templates_visible !== 0 : Boolean(req.body.portal_templates_visible);
    const templateInputsEnabled = templatesVisible && (req.body?.portal_template_inputs_enabled === undefined ? client.portal_template_inputs_enabled !== 0 : Boolean(req.body.portal_template_inputs_enabled));
    await firmDb.prepare(`
      UPDATE firm_clients
      SET
        ...
        updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `).run(templatesVisible ? 1 : 0, templateInputsEnabled ? 1 : 0, client.id);
    const updatedClient = await firmDb.prepare('SELECT * FROM firm_clients WHERE id = ?').get(client.id);
    return res.json({
...
firmsRouter.post('/:id/clients/:clientId/email', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  clientParam: 'clientId'
}), async (req, res, next) => {
...
firmsRouter.post('/:id/document-requests/:requestId/upload', requireFirmAccess({
  write: true
}), requireAssignedMatter({
  requestParam: 'requestId'
}), clientDocumentUpload.single('document'), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  ...
  const firmDb = await openFirmDatabase(firm);
  try {
    const request = await firmDb.prepare('SELECT * FROM client_document_requests WHERE id = ?').get(req.params.requestId);
    if (!request) return res.status(404).json({
      error: 'Document request not found'
    });
    const uploadResult = await firmDb.prepare(`
      INSERT INTO client_uploads
        (request_id, client_id, original_filename, stored_filename, mime_type, file_size)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(request.id, request.client_id, req.file.originalname, req.clientUploadStoredFilename || req.file.filename, req.file.mimetype, req.file.size);
    await firmDb.prepare(`
      UPDATE client_document_requests
      SET status = 'uploaded', updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `).run(request.id);
    if (!(await hasPendingDocuments(firmDb, request.client_id))) {
      await firmDb.prepare(`
        UPDATE firm_clients
    ...
    res.status(201).json(await getWorkspacePayload(firm, req));
  } finally {
    await firmDb.close();
  }
});
firmsRouter.get('/:id/uploads/:uploadId/download', requireFirmAccess(), async (req, res) => {
  const firm = await getFirmOr404(req.params.id, res);
  if (!firm) return;
  const firmDb = await openFirmDatabase(firm);
  try {
    const upload = await firmDb.prepare('SELECT * FROM client_uploads WHERE id = ?').get(req.params.uploadId);
    if (!upload) return res.status(404).json({
...
firmsRouter.patch('/:id', requireAdminOnly, async (req, res) => {
  const {
    name,
    contact_name: contactName,
    contact_email: contactEmail,
  ...
  const result = await db.prepare(`
    ...
    SET
      ...
      updated_at = CURRENT_TIMESTAMP
    WHERE id = ?
  `).run(String(name).trim(), contactName ? String(contactName).trim() : null, contactEmail ? String(contactEmail).trim().toLowerCase() : null, contactPhone ? String(contactPhone).trim() : null, address ? String(address).trim() : null, req.params.id);
  if (result.changes === 0) return res.status(404).json({
    error: 'Firm not found'
  });
  const firm = await db.prepare('SELECT * FROM firms WHERE id = ?').get(req.params.id);
  await initializeFirmDatabase(firm);
  res.json({
    firm: serializeFirm(firm)
  });
});
firmsRouter.patch('/:id/status', requireAdminOnly, async (req, res) => {
  const {
    status
  } = req.body || {};
...
firmsRouter.delete('/:id', requireAdminOnly, async (req, res) => {
  const firm = await db.prepare('SELECT * FROM firms WHERE id = ?').get(req.params.id);
  if (!firm) return res.status(404).json({
    error: 'Firm not found'
  });
  await dropFirmDatabase(firm);
  await db.prepare('DELETE FROM firms WHERE id = ?').run(req.params.id);
  res.json({
    deleted: true,
    firm: serializeFirm(firm)
  });
