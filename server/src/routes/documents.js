...
documentsRouter.get('/:id', authenticate, async (req, res) => {
  ...
  const fields = (await getTemplateFields(row.template_id)).map(serializeField);
  return res.json({
    document: serializeDocument(row),
    fields
  });
});
documentsRouter.get('/:id/download', authenticate, async (req, res) => {
  const row = await db.prepare(`
    SELECT *
    FROM generated_documents
  ...
  res.setHeader('Content-Disposition', `${req.query.inline ? 'inline' : 'attachment'}; filename="${row.file_name}"`);
